...
class Node 
{
    public:
        ...
        // Constructors
        Node(float x, float y, int* robot_tau, int* robot_id) : x(x), y(y)
        {
...
class Robot
{
    public:
        ...
        int robot_id = 0; 
        std::queue <Node> reltaive_position;
};
...
class Rect {
    public:
        ...
        {
            return point.x >= west_edge && point.x < east_edge &&
                point.y >= north_edge && point.y < south_edge;
        }
...
public:
    QuadTree(Rect boundary, int max_points = 4, int sensor_range = 10, int depth = 0)
    : boundary(boundary), max_points(max_points), depth(depth), sensor_range(sensor_range), divided(false) {
    ...
    /**
     * Divides the current QuadTree node into four child nodes.
     *
     ...
     * and southwest) and initializes a child QuadTree for each quadrant with
     * the new boundaries. This division allows the QuadTree to maintain a balanced
     * distribution of points across its nodes, enhancing spatial query performance.
     *
     * Each child QuadTree inherits the max_points limit and sensor range from
    ...
    void divide() 
    {
        ...
        // Create child QuadTree nodes for each quadrant with the respective boundaries.
        nw = std::make_unique<QuadTree>(nwBoundary, max_points, sensor_range, depth + 1);
        ne = std::make_unique<QuadTree>(neBoundary, max_points, sensor_range, depth + 1);
        se = std::make_unique<QuadTree>(seBoundary, max_points, sensor_range, depth + 1);
        sw = std::make_unique<QuadTree>(swBoundary, max_points, sensor_range, depth + 1);

        divided = true; // Indicate that this node has now been divided into child nodes.
    }

    /**
     * Attempts to insert a point into the QuadTree, traversing all the way to the
     ...
     */
    bool query_circle(const Rect& boundary, const Node& centre, float radius, std::vector<Node>& found_nodes) {
        ...
        bool found = false;
        // Search this node's nodes to see if they lie within the circular boundary.
        for (const auto& point : points) {
            if (boundary.contains(point) && point.distanceTo(centre) <= radius) {
                found_nodes.push_back(point);
                found = true;
            }
        }
    ...
    /**
     * Finds nodes within the QuadTree that lie within a specified radius of a given center node.
     *
    ...
    {
        // Calculate the bounding box for the search circle.
        float centerX = centre.x;
        float centerY = centre.y;
        Rect boundary(centerX - radius, centerY - radius, 2 * radius, 2 * radius);

        // Call query_circle with the calculated boundary, center, and radius.
        return query_circle(boundary, centre, radius, found_nodes);
    }


    /**
     * Calculates the total number of points contained within this QuadTree node and all of its subdivisions.
     * 
     * This method recursively traverses the QuadTree, starting from this node and including all its child nodes if any,
     ...
     * 
     * The method ensures an accurate count of all points within the encompassed spatial region of the QuadTree,
     * providing a means to understand the density or distribution of points managed by the QuadTree structure.
     * 
     * @return The total count of points within this QuadTree node and its child nodes.
     */
    int tree_size() const 
    {
